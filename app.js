...
const saveKey = 'familyquest-retrone-v2';
...
const defaultState = {
  started:false, current:1, completed:[], xp:0, level:1, photos:{},
  secretsUnlocked:[], bonusRewards:[], scanStrict:false
};
let state = loadState();
...
function reset(){ if(confirm('Vuoi davvero azzerare la missione?')){ state={...defaultState}; save(); render(); } }
...
function missionImg(m,type='wide'){
...
function shell(content){
  return `<button class="audio-toggle" onclick="startMusic()">${audioOn?'🎵':'🔇'}</button>
  <div class="topbar">
...
function home(){
  ...
  return `<section class="hero"><div class="card">
    ...
    <p class="subtitle">9 missioni, scanner fotografico, musica, segreti, premi casuali, chiavi, frammenti e un tesoro finale nel mondo vero.</p>
    ...
    <button class="btn secondary" onclick="view='inventory';state.started=true;save();render()">Apri archivio</button>
  </div></section>`;
}
...
function mission(){
  ...
  return `<section class="grid">
    <div class="card">
      ...
      <img class="mission-img" src="${missionImg(m,'wide')}" onerror="this.style.display='none'">
      ...
      <div class="actions">
        ...
        <button class="btn secondary" onclick="showTarget(${m.id})">Aiutino</button>
      </div>
      <p class="small">🔐 Lettera segreta della missione: si sblocca dopo la foto.</p>
    </div>
    <div class="card">
      ...
      <div class="secretBox">
        <input id="secretInput" placeholder="Codice segreto">
        <button class="btn secondary" onclick="unlockSecret()">OK</button>
      </div>
    </div>
  </section>`;
}
function showTarget(id){ const m=adventure.missions.find(x=>x.id===id); alert(`Obiettivo da fotografare:\n\n${m.target}`); }
...
async function scanPhoto(id, dataUrl){
  app.insertAdjacentHTML('beforeend', `<div class="scan"><div class="scanbox card">
    ...
    <p class="small" id="scanText">Caricamento impronta visiva locale...</p>
  ...
  setTimeout(()=>{
    const ok = score > 0.45 || !state.scanStrict;
    const scan = $('.scan');
    if(ok){
      scan.querySelector('#scanText').innerHTML = `<span class="result-badge">Oggetto riconosciuto ${(score*100|0)}%</span>`;
      setTimeout(()=>completeMission(id, score),900);
    } else {
      failSound(); vibrate([80,60,80]);
      scan.querySelector('.scanbox').classList.add('shake');
      scan.querySelector('#scanText').innerHTML = `<span class="result-badge fail-badge">Non sono sicura: ${(score*100|0)}%</span><br><br><button class="btn" onclick="document.querySelector('.scan').remove(); takePhoto(${id})">Riprova</button><button class="btn secondary" onclick="completeMission(${id}, ${score})">Sblocca comunque</button>`;
    }
  },2400);
}

// Simple local image similarity: average color histogram + luminance hash
...
function completeMission(id, score=0.75){
  ...
  app.insertAdjacentHTML('beforeend', `<div class="reward"><div class="reward-card">
    ...
    <h2>${m.success}</h2>
    <p><b>${m.key}</b><br>✨ ${m.fragment}<br>🏅 ${m.badge}<br>🎁 Premio casuale: ${bonus}<br>🔐 Segno segreto: ${m.secretLetter || '★'}<br>+${m.xp} XP</p>
...
function inventory(){
  return `<section class="grid">
    <div class="card"><div class="kicker">Archivio</div><h2>Inventario</h2><p class="small">Livello ${state.level}: ${levelName(state.level)} · ${state.xp} XP</p>
      ${mascot('Qui tengo al sicuro chiavi, frammenti, badge, premi casuali e codici segreti. Non perdo nulla. Sono una volpe professionista.')}
    </div>
    ...
    <div class="card"><h3>🔐 Codici segreti sbloccati</h3>${state.secretsUnlocked.length?state.secretsUnlocked.map(c=>`<div class="result-badge">${c}</div>`).join(''):'<p class="small">Prova a scoprire i codici nascosti.</p>'}</div>
  </section>`;
}
function album(){
